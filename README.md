...
Imported sessions show up in the Replay picker alongside native ones. By default `import-jsonl` writes all observations first, then reindexes the imported session once and persists that index. Under the hood each entry routes through the `mem::replay::load`, `mem::replay::sessions`, and `mem::replay::import-jsonl` iii functions — no side-channel servers.

For bulk backfills where you plan to run full extraction later, skip import-time session indexing and the lightweight replay lesson/crystal pass:

```bash
npx @agentmemory/agentmemory import-jsonl <jsonl-file-or-directory> --manual-index --no-lesson-extraction
npx @agentmemory/agentmemory finalize-replay-index
```

Run `finalize-replay-index` only after all bulk import batches have succeeded. The older `--defer-index` / `--deferred-index` flags are not accepted; use `--manual-index`.
...
150 endpoints on port `3111`. The REST API binds to `127.0.0.1` by default. Protected endpoints require `Authorization: Bearer <secret>` when `AGENTMEMORY_SECRET` is set, and mesh sync endpoints require `AGENTMEMORY_SECRET` on both peers.
...
- `POST /agentmemory/semantic-rollup` accepts
  ...
  corpus kind is no longer supported because full extraction uses
  `semantic_rollup_full` as the only semantic aggregation stage. Responses
  include `success`, `inputHash`, source provenance, and `semanticMemoryIds` on
  success. Failure responses include `runId`, `windowId`, `mark`, `kind`, and
  `inputHash` so external orchestrators can persist deterministic failed units.
- `POST /agentmemory/extraction-runs/record` accepts
  `{ runId, mark, status?, stage?, unitId?, sourceIds?, resultIds?, resultType?, summarySessionId?, lessonRunId?, semanticWindowId? }`
