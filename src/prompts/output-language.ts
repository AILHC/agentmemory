...
export interface OutputLanguageFieldContract {
  semantic?: string[];
  lesson?: string[];
  summary?: string[];
  reflect?: string[];
...
export function withOutputLanguagePolicy(
  systemPrompt: string,
  env: Record<string, string | undefined> = process.env,
  contract: OutputLanguageFieldContract = {},
): string {
  ...
  const fields = Object.entries(contract)
    .flatMap(([section, lines]) => {
      const normalized =
        Array.isArray(lines) ? lines.filter((line) => line.trim().length > 0) : [];
      if (normalized.length === 0) return [];
      const title = section.toUpperCase();
      return [
        `${title}:`,
        ...normalized.map((line) => `- ${line.trim()}`),
        "",
      ];
    })
    .join("\n");

  const contractText = fields.length > 0
    ? `
