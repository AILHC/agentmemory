...
async function createFixture(context) {
  ...
  await Promise.all([
    ...
    fs.writeFile(path.join(root, 'scripts', 'lib', 'consolidation-procedural-recovery-adapter-v1.mjs'), 'export {};\n'),
    fs.writeFile(path.join(root, 'scripts', 'lib', 'crystal-recovery-adapter-v1.mjs'), 'export {};\n'),
    ...
    fs.writeFile(path.join(root, 'scripts', 'lib', 'reflect-insight-recovery-adapter-v1.mjs'), 'export {};\n'),
    fs.writeFile(path.join(root, 'scripts', 'lib', 'recoverable-stage-v2.mjs'), 'export {};\n'),
    fs.writeFile(path.join(root, 'scripts', 'lib', 'recovery-frontier-migration-v1.mjs'), 'export {};\n'),
    fs.writeFile(path.join(root, 'scripts', 'lib', 'recovery-journal-reducer-v1.mjs'), 'export {};\n'),
    fs.writeFile(path.join(root, 'scripts', 'lib', 'recovery-migration-contract-v1.mjs'), 'export {};\n'),
    fs.writeFile(path.join(root, 'scripts', 'lib', 'recovery-policy-v1.mjs'), 'export {};\n'),
    fs.writeFile(path.join(root, 'scripts', 'lib', 'recovery-status-projection-v1.mjs'), 'export {};\n'),
    ...
    fs.writeFile(path.join(root, 'scripts', 'lib', 'safe-stage-recovery-adapters-v1.mjs'), 'export {};\n'),
    fs.writeFile(path.join(root, 'scripts', 'lib', 'semantic-rollup-recovery-adapter-v1.mjs'), 'export {};\n'),
...
test('生成清单后可以验证完整 current，且清单不哈希自身', async (context) => {
  ...
  assert.equal(
    manifest.recoveryContract.minVersion,
  ...
  assert.deepEqual(
    ...
    [
      ...
      'scripts/lib/consolidation-procedural-recovery-adapter-v1.mjs',
      'scripts/lib/crystal-recovery-adapter-v1.mjs',
      ...
      'scripts/lib/reflect-insight-recovery-adapter-v1.mjs',
      ...
      'scripts/lib/safe-stage-recovery-adapters-v1.mjs',
      'scripts/lib/semantic-rollup-recovery-adapter-v1.mjs',
